<template>
  <div class="wrapper">
    <div class="menu-btn" @click="toggleActive">
    ...
    <div class="icons-wrapper">
      <div class="icons">
        <div v-for="(icon, index) in icons" :key="index" class="icon" :class="{ show: isActive }" >
          <i :class="`fa ${icon.class}`" :title="icon.title" @click="openPurchaseHistoryModal"></i>
        </div>
      </div>
    </div>
    <PurchaseHistory v-if="showModal" @closeModal="closeModal" />
  </div>
</template>
...
const icons = [
{ class: 'fa-credit-card', title: '결제 내역' }
];
...
border: 1px solid #000000; /* 원형 테두리에 검은선 추가 */
width: 56px;
height: 56px;
border-radius: 50%;
display: flex;
