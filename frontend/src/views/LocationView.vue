    ...
    <div class="map-container">
      ...
      <div class="offcanvas offcanvas-start" data-bs-scroll="true" data-bs-backdrop="false" tabindex="-1" id="offcanvasScrolling" aria-labelledby="offcanvasScrollingLabel" style="width: 410px;  height: calc(100vh - 81px); bottom: 0; top: auto; border: none;">
        ...
        <div class="offcanvas-body">
          ...
          <YoutubeCard v-for="youtube in youtubeData" :key="youtube.id" :youtube="youtube" />
        </div>
      </div>
...
import HotelCard from '@/components/search/HotelCard.vue'; //호텔 카드
import RestaurantCard from '@/components/search/RestaurantCard.vue'; //음식점 카드
import AttractionCard from '@/components/search/AttractionCard.vue'; //관광지 카드
import Hamburgermenu from '@/components/search/HamburgerMenu.vue' //햄버거 메뉴
import DrawDirection from "@/components/search/DrawDirection.vue"; //경로(polyline) 그리기
import HotelDate from "@/components/search/HotelDate.vue"; //경로(polyline) 그리기
...
// 이건 크롤링 해서 얻은 호텔 리스트 나중에 필요하면 사용하기
...
async function searchLocation(places){
  // const photo=new mapRef.value.api.places.Photo()
  // console.log(photo);
  // console.log(placesService.value);
  const location=places.geometry.location
  console.log('장소장소',places.name)
  console.log('제발',getGeoCode(places.geometry.location.lat(),places.geometry.location.lng()))
...
const apiKey='AIzaSyBMpSPfY-brXtLzGQNDvnsJyf-r61u-H6k'; /* 여기에 해당 apikey설정할 것!!!! 'AIzaSyBMpSPfY-brXtLzGQNDvnsJyf-r61u-H6k' */
...
.search-container {
  ...
  padding: 6px;
  border-radius: 30px;
  border: 2px solid black; /* 검은색 실선 외곽선 추가 */
}
